...
def format_help_message(*, lang: str, is_admin: bool, bot_username: str) -> str:
    ...
    if lang == "ru":
        if is_admin:
            return (
                ...
                "• /ii — ответ ИИ на сообщение пользователя (только reply на сообщение, для проверки)\n"
                "• /ping — бот на связи, краткая сводка\n"
                "• /status — статус индекса, лимиты и проверка доступа чата/темы\n"
                "• /stats [YYYY-MM-DD] — дневная сводка группы или текущей темы\n"
                ...
                "Модерация (командой ответьте на сообщение пользователя)\n"
                "• /ban, /unban — заблокировать или разблокировать\n"
                "• /kick — удалить пользователя с возможностью вернуться\n"
                "• /mute [30s|10m|2h|3d] и /unmute — мут (по умолчанию 1 час)\n"
                "• /warn, /unwarn, /warnings, /clearwarns — предупреждения\n"
                "• /del, /pin, /unpin — удалить или закрепить сообщение (ответом на него)\n\n"
    ...
    if is_admin:
        return (
            ...
            "• /ii — ask AI to answer a user message (reply command, admin test only)\n"
            "• /ping — quick health check\n"
            "• /status — index status, limits, chat/topic access check\n"
            "• /stats [YYYY-MM-DD] — daily summary for the group or current topic\n"
            ...
            "Moderation (reply to a user message)\n"
            "• /ban, /unban — ban or unban\n"
            "• /kick — remove a user but allow rejoining\n"
            "• /mute [30s|10m|2h|3d] and /unmute — mute (1 hour by default)\n"
            "• /warn, /unwarn, /warnings, /clearwarns — warnings\n"
            "• /del, /pin, /unpin — delete or pin a message (reply to it)\n\n"
