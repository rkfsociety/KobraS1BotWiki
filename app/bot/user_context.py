...
Три уровня:
  ...
  3. Короткий оперативный контекст чата — последние 15, TTL 10 мин.

Полная история сообщений Telegram хранится в канонической SQLite-базе
``data/chat.sqlite3``; этот модуль оставляет только небольшой быстрый кэш
для обогащения текущего запроса.
