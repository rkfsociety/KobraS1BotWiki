...
предыдущий день в общую тему (`message_thread_id=1`) каждого чата из
`ALLOWED_CHAT_IDS`. Статистика запрашивается отдельно по каждому `chat_id`,
поэтому сообщения разных групп не смешиваются. Если `ALLOWED_CHAT_IDS` не
задан, используется `PANEL_ADMIN_CHAT_ID`.
