...
def _application(
    status: str | None = None, *, error: Exception | None = None, is_member: bool | None = None
):
...
def test_regular_and_active_restricted_members_are_group_members():
    assert asyncio.run(is_group_member(_application(ChatMemberStatus.MEMBER), 42)) is True
    assert asyncio.run(is_group_member(_application(ChatMemberStatus.RESTRICTED, is_member=True), 42)) is True


def test_restricted_member_without_membership_is_denied():
    assert asyncio.run(is_group_member(_application(ChatMemberStatus.RESTRICTED, is_member=False), 42)) is False
