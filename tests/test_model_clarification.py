...
def test_printer_mention_reuses_bounded_cache():
    text = "на kobra s1 не калибруется стол"
    _printer_mentioned.cache_clear()

    assert _printer_mentioned(text)
    before = _printer_mentioned.cache_info()
    assert _printer_mentioned(text)
    after = _printer_mentioned.cache_info()
